...
red4ext_repo_dev_dir   := join(".", "target", "debug")
...
# same as install, in debug mode
dev:
 cargo build
 mkdir -p '{{ join(red4ext_game_dir, mod_name) }}'
 cp -f '{{ join(red4ext_repo_dev_dir, bin_name) }}' '{{ join(red4ext_game_dir, mod_name, bin_name) }}'
 @just hot-reload
...
logs:
 ...
 cat '{{ join(game_dir, "red4ext", "logs", log_name) }}'
